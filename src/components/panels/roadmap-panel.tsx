  ...
  return (
    <View style={{ gap: 18 }}>
            ...
            {open && (
              <View style={styles.steps}>
                {phase.steps.map((step) => (
                  <Step key={step.id} step={step} done={done.includes(step.id)} color={track.color} onToggle={() => toggleRoadmapStep(key, step.id)} />
                ))}
              </View>
            )}
          </Card>
      ...
      <Txt variant="small" style={{ color: C.textFaint }}>
        Links were checked when this roadmap library was built. Course providers can change prices or pages, so confirm
...
function Step({ step, done, color, onToggle }: { step: RoadmapStep; done: boolean; color: string; onToggle: () => void }) {
  return (
    <View style={[styles.step, done && { opacity: 0.72 }]}>
      ...
      <View style={{ gap: 6, marginLeft: 32 }}>
        {step.resources.map((r) => (
          <Pressy key={r.id} onPress={() => WebBrowser.openBrowserAsync(r.url)} style={styles.resource}>
            <View style={styles.resourceIcon}>
              <Ionicons name={TYPE_ICON[r.type].icon} size={15} color={r.type === 'video' ? '#FF4E45' : C.cyan} />
            </View>
            <View style={{ flex: 1 }}>
              ...
              <Txt variant="small" numberOfLines={1}>
                {r.provider} · {TYPE_ICON[r.type].label} · {r.duration}
              </Txt>
            </View>
